...
phrase_duration=0
begin_time = None
if not os.path.exists(f'../wav_{fs}Hz_phrase'):
    os.mkdir(f'../wav_{fs}Hz_phrase')
    ...
    with open(lab_file,'r') as lab:
        ...
        for line in lab.readlines():
            ...
            if p == 'pau' and len(phrase_phoneme)>0:
                ...
                cmd = 'sox '+wav_file+f' -r {fs} -b 16 -c 1 ../VISinger_data/wav_{fs}Hz_phrase/'+file_name+'_'+str(num).zfill(3)+'_bits16.wav'+' trim '+str(begin_time)+' '+str(phrase_duration)
                print(cmd)
